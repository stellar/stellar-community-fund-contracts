...
pub(crate) fn read_votes_admin_contract_address(env: &Env) -> Address {
    env.storage()
        .instance()
...
pub(crate) fn write_votes_admin_contract_address(env: &Env, admin: &Address) {
    env.storage()
        .instance()
        .set(&DataKey::VotesAdminAddress, &admin);
}
