...
impl TrustHistoryNeuron {
    pub fn from_data(round: usize, trust_graph_neurons_results: HashMap<String, HashMap<String, f64>>) -> Self {
        Self {
            round,
...
impl Neuron for TrustHistoryNeuron {
    ...
    fn calculate_result(&self, _users: &[String]) -> HashMap<String, f64> {
        ...
        for i in self.round - 1..=self.round {
            let user_trust: HashMap<String, f64> =
                self.trust_graph_neurons_results.get(&format!("trust_graph_neuron_{}", i)).unwrap().clone();
